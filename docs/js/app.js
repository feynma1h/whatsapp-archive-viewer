...
/* ---- picking files ----
   A phone browser copies the whole archive into its own storage before it
   hands the file over, which takes minutes for a multi-gigabyte export. The
   page can neither report that wait nor be reached during it: the system
   keeps its own picker on screen until the copy is done, and the file arrives
   in the same moment the page comes back. All that is left to handle is the
   result — a copy that fails part-way reports success with nothing attached. */
